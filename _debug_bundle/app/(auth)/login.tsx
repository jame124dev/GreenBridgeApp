      ...
      onError: (err) => {
        if (err instanceof LoginError) {
          if (err.code === 'EMAIL_NOT_VERIFIED') {
            return Alert.alert(t('mobile.auth.verifyEmailTitle'), err.message);
          }
          ...
          if (err.code === 'BUYER_NOT_ALLOWED') {
            return Alert.alert(t('mobile.auth.sellersOnlyTitle'), err.message);
          }
    ...
    <Screen padded={false} keyboardAware edges={[]} contentContainerStyle={{ paddingBottom: 0 }}>
      ...
      <Card variant="flat" className="mt-[-28px] mx-lg px-lg py-xl gap-xl">
        ...
        <Pressable
          onPress={() =>
            Alert.alert(
              t('mobile.auth.forgotPasswordTitle'),
              t('mobile.auth.forgotPasswordBody'),
            )
          }
